...
const REACT_LAB_PATH = "/reference/evidence/prototypes/aha-soft-skeuomorphic-hover-system-2026-07-06/";
const REACT_LAB_DIST = path.join(process.cwd(), REACT_LAB_PATH.slice(1), "dist");
const PAGE_TRANSITION_LAB_PATH = "/reference/evidence/prototypes/aha-page-transition-playground-2026-08-28/";
const PAGE_TRANSITION_LAB_DIST = path.join(process.cwd(), "reference/evidence/prototypes/aha-living-gradient-playground-2026-06-26/page-transition/dist");
const PARALLAX_LAB_PATH = "/reference/evidence/prototypes/aha-parallax-scroll-storytelling-playground-2026-07-20/";
const PARALLAX_LAB_DIST = path.join(process.cwd(), PARALLAX_LAB_PATH.slice(1), "dist");
const VISUAL_LANGUAGE_CONFIG = path.join(process.cwd(), "reference/evidence/prototypes/aha-soft-skeuomorphic-hover-system-2026-07-06/src/config/visual-language.json");
const VISUAL_LANGUAGE_BODY_CONTROLS = path.join(process.cwd(), "reference/evidence/prototypes/aha-soft-skeuomorphic-hover-system-2026-07-06/src/config/visual-language-body-controls.json");
const VISUAL_LANGUAGE_BODY_SAVE_ENDPOINT = "/__visual-language-lab/save-bodies";
...
const ADAPTIVE_CARD_CONFIG = path.join(process.cwd(), "reference/evidence/prototypes/aha-spatial-tabs-cards-playground-2026-06-29/config.json");
const ADAPTIVE_CARD_SAVE_ENDPOINT = "/__adaptive-card-lab/save-settings";
const KEN_BURNS_CONFIG = path.join(process.cwd(), "reference/evidence/prototypes/aha-ken-burns-background-playground-2026-06-25/config.json");
const KEN_BURNS_SAVE_ENDPOINT = "/__ken-burns-lab/save-settings";
const PARALLAX_STORY_CONFIG = path.join(process.cwd(), "reference/evidence/prototypes/aha-parallax-scroll-storytelling-playground-2026-07-20/src/config/parallax-story.json");
const PARALLAX_STORY_CONTROLS = path.join(process.cwd(), "reference/evidence/prototypes/aha-parallax-scroll-storytelling-playground-2026-07-20/src/config/parallax-story-controls.json");
const PARALLAX_STORY_SAVE_ENDPOINT = "/__parallax-story-lab/save-settings";
const ADAPTIVE_CARD_VALUE_RANGES = {
  targetContrast: [4.5, 12],
  sampleDepth: [0.28, 0.7],
  toneSaturation: [0.45, 1.4],
  gradientHeight: [44, 78],
  gradientBlur: [0, 14],
};
const KEN_BURNS_VALUE_RANGES = {
  holdDuration: [4000, 30000],
  motionDuration: [6000, 36000],
  fadeDuration: [0, 3000],
  overscan: [4, 20],
  zoomAmount: [0.04, 0.2],
  panStrength: [0.25, 1.5],
};
// The UI and managed save endpoint deliberately read the same bounds. This prevents a slider
// change from becoming saveable in one layer but rejected (or silently clamped) in the other.
const PARALLAX_STORY_VALUE_RANGES = Object.fromEntries(
  JSON.parse(fs.readFileSync(PARALLAX_STORY_CONTROLS, "utf8"))
    .flatMap((group) => group.controls)
    .map((control) => [control.key, [control.min, control.max]]),
);
const VISUAL_LANGUAGE_BODY_CONTROL_GROUPS = JSON.parse(fs.readFileSync(VISUAL_LANGUAGE_BODY_CONTROLS, "utf8"));
const VISUAL_LANGUAGE_BODY_VALUE_RANGES = Object.fromEntries(
  VISUAL_LANGUAGE_BODY_CONTROL_GROUPS.flatMap((group) => group.controls.map((control) => [
...
async function saveAdaptiveCardSettings(request, response) {
  try {
    const body = JSON.parse(await readRequestBody(request));
    const state = {};

    Object.entries(ADAPTIVE_CARD_VALUE_RANGES).forEach(([key, [min, max]]) => {
      const value = Number(body?.state?.[key]);
      if (Number.isFinite(value)) state[key] = Math.min(max, Math.max(min, value));
    });

    if (Object.keys(state).length !== Object.keys(ADAPTIVE_CARD_VALUE_RANGES).length) {
      sendJson(response, 400, { saved: false, message: "State payload is missing required adaptive card values." });
      return;
    }

    const payload = {
      schema: "aha-adaptive-colour-card/v1",
      surface: "AHA Adaptive Colour Card Lab",
      state,
    };
    fs.writeFileSync(ADAPTIVE_CARD_CONFIG, `${JSON.stringify(payload, null, 2)}\n`);
    sendJson(response, 200, { saved: true, message: "Updated the adaptive colour card config." });
  } catch (error) {
    sendJson(response, 500, { saved: false, message: error.message });
  }
}
...
async function saveKenBurnsSettings(request, response) {
  try {
    const body = JSON.parse(await readRequestBody(request));
    const state = {};

    Object.entries(KEN_BURNS_VALUE_RANGES).forEach(([key, [min, max]]) => {
      const value = Number(body?.state?.[key]);
      if (Number.isFinite(value)) state[key] = clampNumber(value, min, max);
    });

    if (Object.keys(state).length !== Object.keys(KEN_BURNS_VALUE_RANGES).length) {
      sendJson(response, 400, { saved: false, message: "State payload is missing required Ken Burns values." });
      return;
    }

    const payload = {
      schema: "aha-ken-burns-background/v1",
      surface: "AHA Ken Burns Background Lab",
      state,
    };
    fs.writeFileSync(KEN_BURNS_CONFIG, `${JSON.stringify(payload, null, 2)}\n`);
    sendJson(response, 200, { saved: true, message: "Updated the Ken Burns config." });
  } catch (error) {
    sendJson(response, 500, { saved: false, message: error.message });
  }
}

async function saveParallaxStorySettings(request, response) {
  try {
    const body = JSON.parse(await readRequestBody(request));
    const state = {};

    Object.entries(PARALLAX_STORY_VALUE_RANGES).forEach(([key, [min, max]]) => {
      const value = Number(body?.state?.[key]);
      if (Number.isFinite(value)) state[key] = clampNumber(value, min, max);
    });

    if (Object.keys(state).length !== Object.keys(PARALLAX_STORY_VALUE_RANGES).length) {
      sendJson(response, 400, { saved: false, message: "State payload is missing required parallax story values." });
      return;
    }

    // Start and full-in are viewport coordinates measured from the top. Keep a 1vh span so
    // scroll progress remains ordered and never divides by zero after a hand-edited request.
    state.backdropTitleFullVh = Math.min(state.backdropTitleFullVh, state.backdropTitleStartVh - 1);
    state.pageTitleFullVh = Math.min(state.pageTitleFullVh, state.pageTitleStartVh - 1);

    const payload = {
      schema: "aha-parallax-scroll-storytelling/v5",
      surface: "AHA Parallax Scroll Storytelling Lab",
      state,
    };
    fs.writeFileSync(PARALLAX_STORY_CONFIG, `${JSON.stringify(payload, null, 2)}\n`);
    sendJson(response, 200, { saved: true, message: "Updated the parallax scroll storytelling config." });
  } catch (error) {
    sendJson(response, 500, { saved: false, message: error.message });
  }
}

async function saveVisualLanguageBodies(request, response) {
  try {
    const body = JSON.parse(await readRequestBody(request));
    const bodies = {};
    let valueCount = 0;

    Object.entries(VISUAL_LANGUAGE_BODY_VALUE_RANGES).forEach(([pathKey, [min, max]]) => {
      const [bodyId, key] = pathKey.split(".");
      const value = Number(body?.bodies?.[bodyId]?.[key]);
      if (!Number.isFinite(value)) return;
      bodies[bodyId] ??= {};
      bodies[bodyId][key] = clampNumber(value, min, max);
      valueCount += 1;
    });

    if (valueCount !== Object.keys(VISUAL_LANGUAGE_BODY_VALUE_RANGES).length) {
      sendJson(response, 400, { saved: false, message: "Body payload is missing required edge values." });
      return;
    }

    const source = fs.readFileSync(VISUAL_LANGUAGE_CONFIG, "utf8");
    const bodiesStart = source.indexOf('"bodies":');
    const open = source.indexOf("{", bodiesStart);
    const close = findBalancedBlock(source, open);
    if (bodiesStart === -1 || open === -1 || close === -1) {
      throw new Error("Could not find the authored body config block.");
    }
    const formattedBodies = JSON.stringify(bodies, null, 2).replaceAll("\n", "\n  ");
    const nextSource = `${source.slice(0, open)}${formattedBodies}${source.slice(close + 1)}`;
    fs.writeFileSync(VISUAL_LANGUAGE_CONFIG, nextSource);
    sendJson(response, 200, { saved: true, message: "Updated the authored Body 1, Body 2, and Body 3 edge config." });
  } catch (error) {
    sendJson(response, 500, { saved: false, message: error.message });
  }
}
...
function serveStatic(request, response) {
  ...
  if (pathname === REACT_LAB_PATH.slice(0, -1)) pathname = REACT_LAB_PATH;
  if (pathname === PAGE_TRANSITION_LAB_PATH.slice(0, -1)) pathname = PAGE_TRANSITION_LAB_PATH;
  if (pathname === PARALLAX_LAB_PATH.slice(0, -1)) pathname = PARALLAX_LAB_PATH;

  let filePath;
  if (pathname.startsWith(REACT_LAB_PATH)) {
    const relativePath = pathname.slice(REACT_LAB_PATH.length);
    filePath = path.normalize(path.join(REACT_LAB_DIST, relativePath || "index.html"));
  } else if (pathname.startsWith(PAGE_TRANSITION_LAB_PATH)) {
    const relativePath = pathname.slice(PAGE_TRANSITION_LAB_PATH.length);
    filePath = path.normalize(path.join(PAGE_TRANSITION_LAB_DIST, relativePath || "index.html"));
  } else if (pathname.startsWith(PARALLAX_LAB_PATH)) {
    const relativePath = pathname.slice(PARALLAX_LAB_PATH.length);
    filePath = path.normalize(path.join(PARALLAX_LAB_DIST, relativePath || "index.html"));
  } else {
    filePath = path.normalize(path.join(process.cwd(), pathname));
  }
...
function runServer() {
  const server = http.createServer((request, response) => {
    if (request.method === "POST" && request.url?.startsWith(VISUAL_LANGUAGE_BODY_SAVE_ENDPOINT)) {
      void saveVisualLanguageBodies(request, response);
      return;
    }
    if (request.method === "POST" && request.url?.startsWith(PARALLAX_STORY_SAVE_ENDPOINT)) {
      void saveParallaxStorySettings(request, response);
      return;
    }
    if (request.method === "POST" && request.url?.startsWith(KEN_BURNS_SAVE_ENDPOINT)) {
      void saveKenBurnsSettings(request, response);
      return;
    }
    if (request.method === "POST" && request.url?.startsWith(ADAPTIVE_CARD_SAVE_ENDPOINT)) {
      void saveAdaptiveCardSettings(request, response);
      return;
    }
...
async function main() {
  ...
  if (!noOpen) {
    try {
      openUrls(urls);
      console.log("Opened AHA Design System Code Demos.");
    } catch {
