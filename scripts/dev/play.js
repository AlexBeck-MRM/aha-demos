...
const adaptiveColourSettings = require("../../reference/evidence/prototypes/aha-spatial-tabs-cards-playground-2026-06-29/settings.js");
...
async function saveAdaptiveCardSettings(request, response) {
  try {
    ...
    if (!body?.state || typeof body.state !== "object" || Array.isArray(body.state)) {
      sendJson(response, 400, { saved: false, message: "An adaptive colour state object is required." });
      return;
    }
    const migrated = adaptiveColourSettings.migrateConfig(body);
    const payload = {
      schema: adaptiveColourSettings.schema,
      surface: "AHA Adaptive Image Colour Lab",
      state: migrated.state,
    };
    fs.writeFileSync(ADAPTIVE_CARD_CONFIG, `${JSON.stringify(payload, null, 2)}\n`);
    sendJson(response, 200, { saved: true, message: "Updated both adaptive colour families.", migrationNotes: migrated.notes });
  } catch (error) {
    sendJson(response, 500, { saved: false, message: error.message });
  }
}
