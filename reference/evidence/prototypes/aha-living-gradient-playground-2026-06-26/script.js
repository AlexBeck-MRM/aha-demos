const STORAGE_KEY = "aha-living-gradient-playground:v44";
const LEGACY_STORAGE_KEYS = ["aha-living-gradient-playground:v43", "aha-living-gradient-playground:v42", "aha-living-gradient-playground:v41", "aha-living-gradient-playground:v40", "aha-living-gradient-playground:v39", "aha-living-gradient-playground:v38", "aha-living-gradient-playground:v37", "aha-living-gradient-playground:v36", "aha-living-gradient-playground:v35"];
const CONFIG_SCHEMA = "aha-living-gradient-playground/v44";
...
const LOGO_EXPORT_MASK_URL = "../../../assets/aha-heart-torch.svg";
...
const presets = [
  {
    ...
    values: {
      ...
      exportExposure: 0.88,
      exportShadowMix: 0.18,
      exportOrangeCap: 0.82,
...
const authoredDefaultValues = {
  ...
  shaderBlur: 22,
  surfaceBlurBoost: 126,
  exportExposure: EXPORT_COLOR_GRADE_DEFAULTS.exposure,
  exportShadowMix: EXPORT_COLOR_GRADE_DEFAULTS.shadowMix,
  exportOrangeCap: EXPORT_COLOR_GRADE_DEFAULTS.orangeLuminanceCap,
  exportSaturation: EXPORT_COLOR_GRADE_DEFAULTS.saturation,
  deepColor: "#520208",
  redColor: "#e2001e",
  orangeColor: "#ffc139",
...
const controlGroups = [
  {
    ...
    icon: "target",
    open: true,
    keys: ["flameScale", "flameRotation", "flameX", "flameY", "edgeSpread", "shaderBlur", "surfaceBlurBoost"],
  },
  {
    id: "edge-finish",
    title: "Edge Finish",
    icon: "plume",
    open: true,
    keys: ["edgeThickness", "edgeSoftness", "edgeMotion", "turbulence", "noiseScale"],
  },
  {
    id: "light-shadow",
    title: "Light & Shadow",
    icon: "beam",
    open: true,
    keys: ["edgeLight", "edgeLightX", "edgeLightY", "edgeShadow", "shadowSoftness", "deepPressure"],
  },
  {
    id: "motion",
    title: "Motion",
    icon: "wave",
    open: true,
    keys: ["duration", "evolutionSpeed", "rise", "sway", "redExtraSway"],
  },
  {
    id: "plume-shape",
    title: "Plume Shape",
    icon: "plume",
    open: true,
    keys: ["flameWidth", "flameHeight", "flameStrength", "redPlumeX", "redPlumeY", "taperPower", "warmLight", "warmSpread", "orangePlumeX", "orangePlumeY", "orangePlumeHeight", "orangePlumeSoftness"],
  },
  {
    id: "colour",
    title: "Colour & Export",
    icon: "swatch",
    open: true,
    keys: ["deepColor", "redColor", "orangeColor", "colorIntensity", "shaderContrast", "figmaExportGrade", "exportExposure", "exportShadowMix", "exportOrangeCap", "exportSaturation"],
  },
  {
    id: "logo-mapping",
    title: "Logo Mapping",
...
const controlDefinitions = {
  ...
  edgeSpread: { key: "edgeSpread", label: "Edge spread", type: "range", min: 0.5, max: 4, step: 0.05, default: authoredDefaultValues.edgeSpread, unit: "x" },
  edgeThickness: { key: "edgeThickness", label: "Edge thickness", type: "range", min: 0.45, max: 2.25, step: 0.01, default: authoredDefaultValues.edgeThickness },
  edgeLight: { key: "edgeLight", label: "Edge light", type: "range", min: 0, max: 0.8, step: 0.01, default: authoredDefaultValues.edgeLight },
  edgeLightX: { key: "edgeLightX", label: "Light X", type: "range", min: -0.5, max: 1.5, step: 0.01, default: authoredDefaultValues.edgeLightX },
  edgeLightY: { key: "edgeLightY", label: "Light Y", type: "range", min: -0.5, max: 1.5, step: 0.01, default: authoredDefaultValues.edgeLightY },
  edgeShadow: { key: "edgeShadow", label: "Edge shadow", type: "range", min: 0, max: 0.9, step: 0.01, default: authoredDefaultValues.edgeShadow },
  shadowSoftness: { key: "shadowSoftness", label: "Shadow softness", type: "range", min: 0.35, max: 2.4, step: 0.01, default: authoredDefaultValues.shadowSoftness },
  edgeMotion: { key: "edgeMotion", label: "Edge motion", type: "range", min: 0, max: 2.5, step: 0.01, default: authoredDefaultValues.edgeMotion },
  ...
  exportExposure: { key: "exportExposure", label: "Export exposure", type: "range", min: 0.62, max: 1.18, step: 0.01, default: authoredDefaultValues.exportExposure },
  exportShadowMix: { key: "exportShadowMix", label: "Export shadow", type: "range", min: 0, max: 0.42, step: 0.01, default: authoredDefaultValues.exportShadowMix },
  exportOrangeCap: { key: "exportOrangeCap", label: "Orange cap", type: "range", min: 0.55, max: 1, step: 0.01, default: authoredDefaultValues.exportOrangeCap },
  exportSaturation: { key: "exportSaturation", label: "Export saturation", type: "range", min: 0.72, max: 1.28, step: 0.01, default: authoredDefaultValues.exportSaturation },
...
const formatters = {
  ...
  edgeSpread: (value) => `${value.toFixed(2)}x`,
  edgeThickness: (value) => value.toFixed(2),
  edgeLight: (value) => value.toFixed(2),
  edgeLightX: (value) => `${Math.round(value * 100)}%`,
  edgeLightY: (value) => `${Math.round(value * 100)}%`,
  edgeShadow: (value) => value.toFixed(2),
  shadowSoftness: (value) => value.toFixed(2),
  edgeMotion: (value) => value.toFixed(2),
  ...
  exportExposure: (value) => value.toFixed(2),
  exportShadowMix: (value) => value.toFixed(2),
  exportOrangeCap: (value) => value.toFixed(2),
  exportSaturation: (value) => value.toFixed(2),
...
function renderPanel() {
  controlsRoot.innerHTML = `${renderPresetOverview()}${controlGroups.map((group) => {
...
function updateDerivedVariables() {
  ...
  prototype.style.setProperty("--lg-edge-spread", state.edgeSpread.toFixed(2));
  prototype.style.setProperty("--lg-edge-thickness", state.edgeThickness.toFixed(2));
  prototype.style.setProperty("--lg-edge-light", state.edgeLight.toFixed(2));
  prototype.style.setProperty("--lg-edge-light-x", `${Math.round(state.edgeLightX * 100)}%`);
  prototype.style.setProperty("--lg-edge-light-y", `${Math.round(state.edgeLightY * 100)}%`);
  prototype.style.setProperty("--lg-edge-shadow", state.edgeShadow.toFixed(2));
  prototype.style.setProperty("--lg-shadow-softness", state.shadowSoftness.toFixed(2));
  prototype.style.setProperty("--lg-edge-motion", state.edgeMotion.toFixed(2));
  ...
  prototype.style.setProperty("--lg-export-exposure", state.exportExposure.toFixed(2));
  prototype.style.setProperty("--lg-export-shadow-mix", state.exportShadowMix.toFixed(2));
  prototype.style.setProperty("--lg-export-orange-cap", state.exportOrangeCap.toFixed(2));
  prototype.style.setProperty("--lg-export-saturation", state.exportSaturation.toFixed(2));
...
const shaderFragmentSource = `
  ...
  uniform float u_edge_spread;
  uniform float u_edge_thickness;
  uniform float u_edge_light;
  uniform float u_edge_light_x;
  uniform float u_edge_light_y;
  uniform float u_edge_shadow;
  uniform float u_shadow_softness;
  uniform float u_edge_motion;
  ...
  vec3 flame(vec2 uv, float t) {
    ...
    float edgeMotion = max(u_edge_motion, 0.0);
    float edgeMotionFactor = edgeMotion <= 1.0
      ? mix(0.35, 1.0, edgeMotion)
      : mix(1.0, 1.45, clamp((edgeMotion - 1.0) / 1.5, 0.0, 1.0));
    float drift = (fbm(vec2(loopSin * 0.36 * loopRadius, loopCos * 0.36 * loopRadius + 0.37)) - 0.5) * 0.18 * u_sway * edgeMotionFactor;
    p.x -= drift;
    ...
    float largeNoise = fbm(vec2(x * 2.0 * noiseScale + loopSin * 0.32 * loopRadius * edgeMotion, vertical * 2.35 * noiseScale + loopCos * 0.6 * loopRadius * edgeMotion));
    float fineNoise = fbm(vec2(x * 4.6 * noiseScale + loopSin * 0.75 * loopRadius * edgeMotion, vertical * 4.4 * noiseScale + loopCos * 0.9 * loopRadius * edgeMotion));
    float motionTurbulence = u_turbulence * edgeMotionFactor;
    float organicEdge = edge - (largeNoise - 0.5) * 0.3 * motionTurbulence - (fineNoise - 0.5) * 0.08 * motionTurbulence;

    float thickness = max(u_edge_thickness, 0.05);
    float thicknessDelta = thickness - 1.0;
    float redStart = clamp((0.56 - u_edge_softness * 0.2) - thicknessDelta * 0.08, 0.18, 1.05);
    float redReach = (0.32 + u_edge_softness * 1.12) * max(u_edge_spread, 0.05) / 2.0;
    ...
    float edgeBand = smoothstep(redStart * 0.72, redStart, organicEdge) * (1.0 - smoothstep(redEnd * 0.72, redEnd, organicEdge)) * activeHeight;
    vec2 lightVector = normalize(vec2(u_edge_light_x, u_edge_light_y) - uv + vec2(0.001));
    float lightFacing = clamp(dot(lightVector, normalize(vec2(-0.45, -0.9))), 0.0, 1.0);
    color = mix(color, u_orange_color, edgeBand * u_edge_light * mix(0.28, 1.0, lightFacing));

    float shadowBand = smoothstep(redStart * 0.42, redEnd * max(u_shadow_softness, 0.05), organicEdge) * activeHeight;
    color = mix(color, u_deep_color, shadowBand * u_edge_shadow);
...
function createShaderRenderer(gl, canvas, program, buffer) {
  return {
    ...
    uniforms: {
      ...
      edgeSpread: gl.getUniformLocation(program, "u_edge_spread"),
      edgeThickness: gl.getUniformLocation(program, "u_edge_thickness"),
      edgeLight: gl.getUniformLocation(program, "u_edge_light"),
      edgeLightX: gl.getUniformLocation(program, "u_edge_light_x"),
      edgeLightY: gl.getUniformLocation(program, "u_edge_light_y"),
      edgeShadow: gl.getUniformLocation(program, "u_edge_shadow"),
      shadowSoftness: gl.getUniformLocation(program, "u_shadow_softness"),
      edgeMotion: gl.getUniformLocation(program, "u_edge_motion"),
...
function getShaderParameters(overrides = null) {
  return {
    ...
    edgeSpread: state.edgeSpread,
    edgeThickness: state.edgeThickness,
    edgeLight: state.edgeLight,
    edgeLightX: state.edgeLightX,
    edgeLightY: state.edgeLightY,
    edgeShadow: state.edgeShadow,
    shadowSoftness: state.shadowSoftness,
    edgeMotion: state.edgeMotion,
...
function drawShaderRenderer(item, shaderTime, width = item.canvas.width, height = item.canvas.height, overrides = null) {
  ...
  gl.uniform1f(item.uniforms.edgeSpread, params.edgeSpread);
  gl.uniform1f(item.uniforms.edgeThickness, params.edgeThickness);
  gl.uniform1f(item.uniforms.edgeLight, params.edgeLight);
  gl.uniform1f(item.uniforms.edgeLightX, params.edgeLightX);
  gl.uniform1f(item.uniforms.edgeLightY, params.edgeLightY);
  gl.uniform1f(item.uniforms.edgeShadow, params.edgeShadow);
  gl.uniform1f(item.uniforms.shadowSoftness, params.shadowSoftness);
  gl.uniform1f(item.uniforms.edgeMotion, params.edgeMotion);
...
function flameCustomProperties() {
  return [
    ...
    ["--lg-edge-spread", state.edgeSpread.toFixed(2)],
    ["--lg-edge-thickness", state.edgeThickness.toFixed(2)],
    ["--lg-edge-light", state.edgeLight.toFixed(2)],
    ["--lg-edge-light-x", `${Math.round(state.edgeLightX * 100)}%`],
    ["--lg-edge-light-y", `${Math.round(state.edgeLightY * 100)}%`],
    ["--lg-edge-shadow", state.edgeShadow.toFixed(2)],
    ["--lg-shadow-softness", state.shadowSoftness.toFixed(2)],
    ["--lg-edge-motion", state.edgeMotion.toFixed(2)],
    ...
    ["--lg-export-exposure", state.exportExposure.toFixed(2)],
    ["--lg-export-shadow-mix", state.exportShadowMix.toFixed(2)],
    ["--lg-export-orange-cap", state.exportOrangeCap.toFixed(2)],
    ["--lg-export-saturation", state.exportSaturation.toFixed(2)],
...
function getExportColorGrade() {
  return {
    ...
    exposure: state.exportExposure,
    shadowMix: state.exportShadowMix,
    orangeLuminanceCap: state.exportOrangeCap,
    saturation: state.exportSaturation,
  };
}
...
function migrateLegacyState(candidate) {
  const next = {
    ...
    edgeSpread: candidate?.edgeSpread ?? authoredDefaultValues.edgeSpread,
    edgeThickness: candidate?.edgeThickness ?? authoredDefaultValues.edgeThickness,
    edgeLight: candidate?.edgeLight ?? authoredDefaultValues.edgeLight,
    edgeLightX: candidate?.edgeLightX ?? authoredDefaultValues.edgeLightX,
    edgeLightY: candidate?.edgeLightY ?? authoredDefaultValues.edgeLightY,
    edgeShadow: candidate?.edgeShadow ?? authoredDefaultValues.edgeShadow,
    shadowSoftness: candidate?.shadowSoftness ?? authoredDefaultValues.shadowSoftness,
    edgeMotion: candidate?.edgeMotion ?? authoredDefaultValues.edgeMotion,
    surfaceBlurBoost: candidate?.surfaceBlurBoost ?? authoredDefaultValues.surfaceBlurBoost,
    exportExposure: candidate?.exportExposure ?? authoredDefaultValues.exportExposure,
    exportShadowMix: candidate?.exportShadowMix ?? authoredDefaultValues.exportShadowMix,
    exportOrangeCap: candidate?.exportOrangeCap ?? authoredDefaultValues.exportOrangeCap,
    exportSaturation: candidate?.exportSaturation ?? authoredDefaultValues.exportSaturation,
...
function getSurfaceBlurMetadata(surface, targetWidth, targetHeight, pixelScale = null, source = "export") {
  ...
  if (!surface) {
    const fallbackPad = getShaderOverscanPad(fallbackBlur);
    return {
      source: "fallback",
  ...
  return {
    ...
    exportWidth: targetWidth,
    exportHeight: targetHeight,
    pad: getShaderOverscanPad(effectiveShaderBlur),
    passCount: getBlurPassCount(rawShaderBlur),
  };
}

function getShaderOverscanPad(blur) {
  return Math.max(0, Math.ceil(blur * clamp(Number(state.edgeSpread) || 0, 0.5, 4)));
}
